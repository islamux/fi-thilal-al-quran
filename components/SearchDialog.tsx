...
export default function SearchDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { results, search, ready } = useSearch();
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh]" onClick={onClose}>
      ...
      <div
        ...
        className="relative w-full max-w-2xl mx-4 bg-surface border border-warm-border rounded-2xl shadow-subtle overflow-hidden anim-scale-in"
      ...
      >
          ...
          <div className="max-h-[50vh] overflow-y-auto p-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
            {results.map((r) => (
              <Link
                ...
                href={`/surah/${r.surah}`}
                onClick={onClose}
                className="block bg-surface border border-warm-border p-3 rounded-xl shadow-subtle hover:shadow-subtle-hover hover:-translate-y-0.5 transition-all duration-200 group active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
              >
        ...
        {input && ready && results.length === 0 && (
          <div className="py-12 text-center text-text-muted">لا توجد نتائج لـ &quot;{input}&quot;</div>
        )}

        {!input && (
          <div className="py-12 text-center text-text-muted text-sm">اكتب كلمة أو عبارة للبحث في تفسير في ظلال القرآن</div>
        )}
      </div>
    </div>
  );
